    ...
    if FLAGS.job_name == "ps":
        ...
        with sv.prepare_or_wait_for_session(server.target) as sess:
            ...
            for epoch in range(TRAINING_EPOCHS):
                ...
                print("start_time:%s    end_time:%s     Final States of w in task%d - epoch%d: " % (start_time,end_time,task_index, epoch), w.eval())

                w_add = w.eval() - init_w
                start_time = str(datetime.datetime.now())
                _ = sess.run(epoch_update_op, feed_dict={w_addup:w_add})
                end_time = str(datetime.datetime.now())
                print("start_time:%s    end_time:%s     target_w: " %(start_time,end_time), target_w.eval())
        sv.stop()
        print("done")
